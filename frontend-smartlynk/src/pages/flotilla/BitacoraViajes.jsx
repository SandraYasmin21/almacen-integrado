...
export default function BitacoraViajes() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("No se pudo cargar la bitácora vehicular.");
  ...
  const exportar = async (format) => {
    try {
      ...
      link.href = url;
      link.download = `bitacora_vehicular.${format === "excel" ? "xlsx" : "pdf"}`;
      link.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Eventos de salida, regreso y kilometraje registrados desde kiosco o administración.</p>
        </div>
